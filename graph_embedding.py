...
def node2vector(graph, type="faster"):
    if type == "faster":
        node2vec = FastNode2Vec(
            n_components=20, walklen=30, return_weight=1, neighbor_weight=1, epochs=50
        )
        node2vec.fit(graph)
        ...
        node2vec = Node2Vec(
            graph, dimensions=20, walk_length=30, num_walks=50, workers=4, p=1, q=1
        )
